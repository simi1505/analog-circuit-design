...
$$
to arrive at an equivalent circuit, given that $A = V_\mathrm{B} / V_\mathrm{A}$ is the voltage gain between nodes A and B. A derivation of this theorem is relatively straightforward considering the current through $Z$ when looking into the impedance from either node A or node B and calculating an equivalent impedance causing the same current.
